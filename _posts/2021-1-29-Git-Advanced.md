...
###### _This is actually not the worst you can find. For fun and entertainment, try doing an image search of "git spaghetti". Photo via on [Attlassian](https://kintosoft.atlassian.net/wiki/plugins/viewsource/viewpagesrc.action?pageId=50213217){:target="\_blank"}._
...
<iframe src="https://giphy.com/embed/Iy0Kg9oUBw9smEKWH8" width="480" height="270" frameBorder="0" class="giphy-embed" allowFullScreen></iframe><p><a href="https://giphy.com/gifs/stickergiant-sticker-giant-Iy0Kg9oUBw9smEKWH8">via GIPHY</a></p>
...
###### _`git rebase [rebaseFromFile]` is a good way to clarify the order of a commit before the final push._

But there are a couple hazards of rebasing. First and foremost, it should not be done in a public repo. Some go as far as to call this [The Golden Rule of Rebasing](https://www.atlassian.com/git/tutorials/merging-vs-rebasing#the-golden-rule-of-rebasing). As noted by the article, this would move all commits to the tip of main/master only for _your_ repository. Fixing this will require a messy merge, so avoiding it is best.
...
In a sense, `git checkout` acts like getting a book from the library: you "checkout" an individual branch. By doing so, you move your working area to that branch, and changes you make will be tracked there. In the same vein, merges you make will be _to_ the branch you've checked out, so to merge or rebase to your main/master, you have to make sure you ```

![Git Checkout](/images/gitCheckout.png)
###### _Switching between branches, or created a new one, made easy by `git checkout`._

You can check out the most recent commit of a branch, or a previous commit based on the commit's absolutely unique ID number, leading to what's reffered to as a detatched HEAD. From there, you can create new branches of the previous commits... but you shouldn't. Instead, you should checkout to the latest branch again. If you want to walk back your commits, you should use `git revert` (see below).
...
You can also directly create a new branch by adding the `-b` option before the branch name to create and immedately check out a new branch. Nifty.

## `git reset` and `git revert`

Finally, there has not a coder that has ever lived that has never totally screwed up something in a commit. So here are a couple ways to fix what you broke, using two Git versions of Ctrl+Z.

Of the two, `git reset` can be more of the nuclear option. There are three different options for `git reset`: `--soft` (which simply moves your commited changed back into the staging area), `--mixed` (which is the default when `git reset` is called, and moves changed back into the working directory), and `--hard`, the truly keys-turning-in-unison, hit-the-big-red-button-option that sends everything you changed right in to the trash.

![Git Reset](/images/gitReset.png)
###### _By `git reset --soft` here, I sent my third commit on the local repo back to the staging area. By re-adding and commiting, it get's sent back again, with now a my newest message in the log and a new commit. The old commit is effectively erased._
...
Instead of excising some portion or all of your commit history for a project on your repo, revert simply creates a new commit at an earlier point in your commit history of your chosing. It does not revise the history, but branches from an earlier point. Like _Avengers Endgame_ (and if you don't remember that, you probably didn't really watch the movie, because boy did they shove it down our throats.)
...
Mainly, Git submodules are useful for providing seperation to a feature or aspect that is subordinant to but somewhat independant from the main module. 
...
You can create submodules manually in your terminal by first initializing git in seperate directory from your main project, and then using `git submodule add ./<submoduleDirectoryName>` to add the repo at submarines to the index of your main repo. GitHub and GitKraken also have tools that let you add and track submodules.
